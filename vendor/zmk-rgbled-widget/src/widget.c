...
// Optional integration with zmk-feature-charge-indicator:
// When CHARGE_INDICATOR is enabled, we suppress LED-off operations while
// charging so the charge-status LED is not interrupted by layer changes
// or activity-sleep events.
#if IS_ENABLED(CONFIG_CHARGE_INDICATOR)
#include <zmk_charge_indicator/charge_indicator.h>
#define IS_CHARGING() zmk_charge_indicator_is_charging()
#else
#define IS_CHARGING() false
#endif
...
void update_layer_color(void) {
    // Do not override layer color while charging; charge_indicator owns the LED
    if (IS_CHARGING()) {
        LOG_DBG("Skipping layer color update while charging");
        return;
    }
...
static int led_layer_color_listener_cb(const zmk_event_t *eh) {
    ...
    if (ev != NULL) {
        ...
        case ZMK_ACTIVITY_SLEEP:
            // Do not turn off LED while charging; charge_indicator needs it on
            if (IS_CHARGING()) {
                LOG_INF("Detected sleep activity state but charging, keeping LED on");
            } else {
